...
icon = pygame.image.load(os.path.join('snake.png'))
pygame.display.set_icon(icon)

fpsClock = pygame.time.Clock()
appl = pygame.image.load(os.path.join('app.png'))
hdImg = pygame.image.load(os.path.join('snakehd.png'))
bod = pygame.image.load(os.path.join('snakebd.png'))
background = pygame.image.load(os.path.join('grass.png'))
tail = pygame.image.load(os.path.join('snaketail.png'))
direction = 'right'
sqr_size = 20
fps = 12
...
def snake(sqr_size, snakelist):
    ...
    if ([snakelist[0][0], snakelist[0][1] + sqr_size] in snakelist) and (
        [snakelist[0][0], snakelist[0][1] - sqr_size] not in snakelist):
        tl = pygame.transform.rotate(tail, 180)
    elif [snakelist[0][0], snakelist[0][1] - sqr_size] in snakelist and [snakelist[0][0],
                                                                         snakelist[0][1] + sqr_size] not in snakelist:
        tl = tail
    elif [snakelist[0][0] + sqr_size, snakelist[0][1]] in snakelist and [snakelist[0][0] - sqr_size,
                                                                         snakelist[0][1]] not in snakelist:
                    ...
                    while stp:
                        for event in pygame.event.get():
                            ...
                            if event.type == pygame.KEYDOWN:
                                if event.key == pygame.K_p:
                                    stp = False
                if event.key == pygame.K_a or event.key == pygame.K_LEFT and sqr_x_change != sqr_mov:
                    direction = 'left'
                    sqr_x_change = -sqr_mov
                    sqr_y_change = 0
                elif event.key == pygame.K_d or event.key == pygame.K_RIGHT and sqr_x_change != -sqr_mov:
                    direction = 'right'
                    sqr_x_change = sqr_mov
                    sqr_y_change = 0
                elif event.key == pygame.K_s or event.key == pygame.K_DOWN and sqr_y_change != -sqr_mov:
                    direction = 'down'
                    sqr_y_change = sqr_mov
                    sqr_x_change = 0
                elif event.key == pygame.K_w or event.key == pygame.K_UP and sqr_y_change != sqr_mov:
                    direction = 'up'
                    sqr_y_change = -sqr_mov
                    sqr_x_change = 0
...
#            applSound = pygame.mixer.Sound(os.path.join('eat_apple.wav'))
#            applSound.play()
            fps += 0.4
            snakeLength += 1
            scr += 1
